...
-- opposite of byp==not byp==matches at least 1 char==can't match empty string
-- Check for proper (cannot recognize epsilon)
proper :: RegExp' -> Bool
proper Zero = True
proper One = False
proper (Let' c) = True
proper (Union' rs) = and [proper x | x <-rs]
proper (Cat' rs) = or [proper x | x <-rs]
proper (Star' r) = False
